...
def graph_data_obj_to_mol_simple(data_x, data_edge_index, data_edge_attr):
    ...
    for i in range(num_atoms):
        ...
        atomic_num = allowable_features['possible_atomic_num_list'][
            atomic_num_idx]
        chirality_tag = allowable_features['possible_chirality_list'][
            chirality_tag_idx]
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for d_path in downstream_dir:
                ...
                downstream_dataset = MoleculeDataset(d_path,
                                                     dataset=dataset_name)
                downstream_smiles = pd.read_csv(os.path.join(
                    d_path, 'processed', 'smiles.csv'),
                                                header=None)[0].tolist()

                assert len(downstream_dataset) == len(downstream_smiles)

                _, _, _, (train_smiles, valid_smiles,
                          test_smiles) = scaffold_split(downstream_dataset,
                                                        downstream_smiles,
                                                        task_idx=None,
                                                        null_value=0,
                                                        frac_train=0.8,
            ...
            downstream_inchi = set(
                pd.read_csv(os.path.join(self.root,
        ...
        # elif self.dataset == ''
            ...
            labels_path = os.path.join(self.root, 'raw',
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    for d in dataset_2:
        old_y = d.y
        new_y = torch.cat(
            [torch.zeros(d_1_y_dim, dtype=torch.long),
             old_y.long()])
        data_list.append(
            Data(x=d.x,
                 edge_index=d.edge_index,
...
class MoleculeFingerprintDataset(data.Dataset):
    ...
    def _process(self):
        ...
        if self.dataset == 'chembl_with_labels':
            ...
            for i in range(len(smiles_list)):
                ...
                rdkit_mol = rdkit_mol_objs[i]
                ## convert aromatic bonds to double bonds
                fp_arr = create_circular_fingerprint(rdkit_mol, self.radius,
        ...
        data_smiles_series.to_csv(os.path.join(processed_dir, 'smiles.csv'),
                                  ...
                                  header=False)
        with open(
                os.path.join(processed_dir, 'fingerprint_data_processed.pkl'),
                'wb') as f:
        ...
        if 'fingerprint_data_processed.pkl' in file_name_list:
            with open(
                    os.path.join(processed_dir,
                                 'fingerprint_data_processed.pkl'), 'rb') as f:
    ...
    def __getitem__(self, index):
        ...
        if hasattr(index, "__iter__"):
            dataset = MoleculeFingerprintDataset(self.root,
                                                 self.dataset,
                                                 self.radius,
                                                 self.size,
...
def _load_chembl_with_labels_dataset(root_path):
    ...
    folds = [
        np.intersect1d(fold, sampleAnnInd.index.values).tolist()
        for fold in folds
    ]
    targetMatTransposed = targetMat[sampleAnnInd[list(
        chain(*folds))]].T.tocsr()
...
def create_all_datasets():
    #### create dataset
    downstream_dir = [
        'bace', 'bbbp', 'clintox', 'esol', 'freesolv', 'hiv', 'lipophilicity',
